public class EliminationAlgorithm {
	...
	private static void find(int num) {
		...
		for (int i = 0; i < 9; i++) {
			for (int j = 0; j < 9; j++) {
				// 如果格子里头有跟要找的数字一样的数字：
				if (Core.sg[i][j].getValue() == num) {
					// 分别用三个代码表示三种排除操作：
					for (int k = 0; k < 3; k++) {
						// 传输要执行的操作代码和该格子的位置信息到exclude()方法，然后标记不可能是答案的格子：
						exclude(k, Core.sg[i][j].getRow(), Core.sg[i][j].getColumn(), Core.sg[i][j].getBlock());
					}
					//下一步：
	...
	private static void exclude(int code, int row, int column, int block) {
		switch (code) {
			...
			case 0:
				for (int i = 0; i < 9; i++) {
					for (int j = 0; j < 9; j++) {
						if (Core.sg[i][j].getRow() == row) {
							Core.sg[i][j].setEditable(false);
						}
					}
				}
				break;
			...
			case 1:
				for (int i = 0; i < 9; i++) {
					for (int j = 0; j < 9; j++) {
						if (Core.sg[i][j].getColumn() == column) {
							Core.sg[i][j].setEditable(false);;
						}
					}
				}
	...
	private static void choose(int block, int num) {
		...
		for (int i = 0; i < 9; i++) {
			for (int j = 0; j < 9; j++) {
				...
				if (Core.sg[i][j].getBlock() == block && Core.sg[i][j].getEditable()) {
					// 记入统计：
					temp++;
				}
			}
